...
| ✅ Done | Same-model panel warning | Delphi / Belief State Diffusion assume *independent* estimators; when one model covers more than half the AI estimator panel the app shows a non-blocking warning (inline at setup + toast at start) and discloses the panel's model composition in the conclusion prompt so convergence claims can be caveated. Pure `consensus/methods/panel_diversity.py` + declarative `DiscussionMethod.assumes_independent_panel` flag (#29). Exact-model grouping; family-level and a "diversify" auto-suggest helper deferred |
...
| ✅ Done | Comprehensive test suite | 2355 tests across 25+ modules covering database, app, config, models, moderator, sessions, tools, documents, MCP client/server, pricing, methods, composable phase handlers, structured outputs, evidence tracking, and panel diversity |
