...
ConsensusApp (app.py) — orchestrator, state management, event emitter
    ...
    │     └── methods/phases/ — 58 reusable handler implementations + helpers
...
- `methods/` — Discussion method subpackage: `base.py` (`DiscussionMethod` ABC, `Phase`, `ProcessedResponse`), `phase_handler.py` (`PhaseHandler` ABC), `parsing.py` (shared parsing utilities), `recommender.py` (`MethodRecommender` — LLM-based method classification engine), 16 method classes including `triage.py` (Guided Triage meta-method), `nominal_group.py` (Nominal Group Technique) and `decision_matrix.py` (Weighted Decision Matrix / MCDA), `phases/` subpackage with 58 composable handler implementations + 10 helper modules
