...
2245 tests in `tests/`. No linter or build system configured yet.
...
ConsensusApp (app.py) — orchestrator, state management, event emitter
    ...
    │     └── methods/phases/ — 67 reusable handler implementations + helpers
...
- `methods/` — Discussion method subpackage: `base.py` (`DiscussionMethod` ABC, `Phase`, `ProcessedResponse`), `phase_handler.py` (`PhaseHandler` ABC), `parsing.py` (shared parsing utilities), `recommender.py` (`MethodRecommender` — LLM-based method classification engine), 18 method classes including `triage.py` (Guided Triage meta-method), `nominal_group.py` (Nominal Group Technique), `decision_matrix.py` (Weighted Decision Matrix / MCDA), `double_crux.py` (Double Crux disagreement resolution) and `tree_of_thoughts.py` (Tree of Thoughts iterative parallel exploration), `phases/` subpackage with 67 composable handler implementations + 13 helper modules
