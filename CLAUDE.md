...
ConsensusApp (app.py) — orchestrator, state management, event emitter
    ...
    │     └── methods/phases/ — 62 reusable handler implementations + helpers
...
- `methods/` — Discussion method subpackage: `base.py` (`DiscussionMethod` ABC, `Phase`, `ProcessedResponse`), `phase_handler.py` (`PhaseHandler` ABC), `parsing.py` (shared parsing utilities), `recommender.py` (`MethodRecommender` — LLM-based method classification engine), 17 method classes including `triage.py` (Guided Triage meta-method), `nominal_group.py` (Nominal Group Technique), `decision_matrix.py` (Weighted Decision Matrix / MCDA) and `double_crux.py` (Double Crux disagreement resolution), `phases/` subpackage with 62 composable handler implementations + 11 helper modules
