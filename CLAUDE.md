...
ConsensusApp (app.py) — orchestrator, state management, event emitter
    ...
    │     └── methods/phases/ — 68 reusable handler implementations + helpers
...
- `background.py` — `spawn_background()`: fire-and-forget asyncio tasks that keep a strong reference and log their exception (used by `tools_document/embedding.py` and `tools_memory.py`)
- `dbkey.py` — `scoped_key()`: scopes a row id to its database file, so process-global bookkeeping cannot collide across `--multi-user` sessions where every session's first row is id 1
...
- `tools_document/` — Document RAG tool provider package: `constants.py`, `errors.py` (`DocumentError`/`DocumentParseError`/`DocumentInterpretationError`/`DocumentIndexError`, each with an actionable `hint`), `parsing.py` (bytes → markdown, URL fetch, section extraction), `chunking.py`, `embedding.py` (cosine/ranking maths + background embedding pass), `schemas.py`, `validation.py` (pure `resolve_range()`/`chapter_range()`), `llm.py`, `ingestion.py` (parse → chunk → store → embed), `handlers.py` (the metadata/navigation `doc_*` handlers), `handlers_rag.py` (the `doc_ask`/`doc_summary` RAG handlers), `provider.py`
...
- `methods/` — Discussion method subpackage: `base.py` (`DiscussionMethod` ABC, `Phase`, `ProcessedResponse`), `phase_handler.py` (`PhaseHandler` ABC), `parsing.py` (shared parsing utilities), `recommender.py` (`MethodRecommender` — LLM-based method classification engine), 18 method classes including `triage.py` (Guided Triage meta-method), `nominal_group.py` (Nominal Group Technique), `decision_matrix.py` (Weighted Decision Matrix / MCDA), `double_crux.py` (Double Crux disagreement resolution) and `tree_of_thoughts.py` (Tree of Thoughts iterative parallel exploration), `phases/` subpackage with 68 composable handler implementations + 15 helper modules
