"""Opening Statements phase handler for Court of Law.

Each side presents their theory of the case.  Teams with multiple
...
class OpeningStatementsHandler(PhaseHandler):
    """Phase 2: Opening statements with optional team huddles."""

    phase = Phase(
        name="opening_statements",
        display_name="Opening Statements",
        description=(
            "Each side presents their theory of the case.  Teams with "
            "multiple members may consult privately before a spokesperson "
            "delivers the opening statement."
        ),
        rounds=1,  # overridden by should_advance — huddle manages rounds
    )

    _huddle_key = "opening_huddle"
    ...
    def get_system_prompt(self, entity: Entity,
        ...
        if sub in ("accusation_huddle", "defense_huddle"):
            ...
            return base + (
                ...
                "conversation.\n\n"
                "Discuss your strategy for the opening statement:\n"
                "- What are the strongest points to lead with?\n"
                "- What narrative or theory should the statement present?\n"
                "- Who should deliver the statement?\n\n"
        ...
        if sub in ("accusation_speaks", "defense_speaks"):
            team_name = label if team == "accusation" else "Defense"
            return base + (
                f"OPENING STATEMENT — {team_name}\n\n"
                "Deliver the opening statement for your team.  "
                "Address the judge directly.  Present your theory of "
                "the case:\n"
                "- Outline what you intend to prove or disprove\n"
                "- Preview the key evidence and arguments\n"
                "- Set the tone for your side of the case\n\n"
                "Be persuasive, clear, and compelling."
            )

        return base + "OPENING STATEMENTS PHASE\n\n"
    ...
    def get_summary_prompt(self, discussion: Discussion,
        ...
        return (
            f"{speaker_name} has delivered an opening statement.  "
            "As presiding judge, note the key claims and theory presented.  "
            f"Next: {next_speaker_name}."
        )
    ...
    def get_transition_message(self, discussion: Discussion) -> str:
        ...
        if acc_count > 1 or def_count > 1:
            parts.append(
                ...
                "before a spokesperson delivers the opening statement.\n"
            )
        parts.append(
            f"The {label} will present first, followed by the Defense."
        )
        return "\n".join(parts)
